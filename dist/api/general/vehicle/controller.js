...
export async function findVehicle(query) {
    ...
    const result = await Vehicle.find(filter).populate("partner_id").populate("current_staff_id").populate("current_partner_id").populate("rating_ids").populate({ path: "created_by", select: "surname other_name email phone" }).populate({ path: "updated_by", select: "surname other_name email phone" }).skip(skip).limit(limit).sort(sort).select(projection).exec();
    return result;
}
