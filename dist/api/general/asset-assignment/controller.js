...
export async function createRecord(req, res) {
    ...
    switch (data.user_type) {
        case "PARTNER":
            UserModel = Partner;userId = "partner_id";break;
        case "STAFF":
            UserModel = Staff;userId = "staff_id";break;
        default:
            return fail(res, 422, `Error invalid user_type ${data.user_type}`);
    }
    let AssetModel;
    let assetId;
    switch (data.asset_type) {
        case "VEHICLE":
            AssetModel = Vehicle;assetId = "vehicle_id";break;
        case "INVENTORY":
            AssetModel = Asset;assetId = "asset_id";break;
        default:
            return fail(res, 422, `Error invalid asset_type ${data.asset_type}`);
    }
    try {
...
export async function updateRecord(req, res) {
    ...
    if (error) return fail(res, 422, `Error validating request data. ${error.message}`);
    try {
        const result = await AssetAssignment.findOneAndUpdate({ _id: id }, data, { new: true });
        if (!result) {
    ...
    try {
        ...
        switch (result.user_type) {
            ...
            case "STAFF":
                UserModel = Staff;userId = "staff_id";break;
            default:
                return fail(res, 422, `Error invalid user_type ${result.user_type}`);
        }
        let AssetModel;
        let assetId;
        switch (result.asset_type) {
            ...
            case "INVENTORY":
                AssetModel = Asset;assetId = "asset_id";break;
            default:
                return fail(res, 422, `Error invalid asset_type ${result.asset_type}`);
        }
