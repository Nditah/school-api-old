...
export async function fetchRecord(req, res) {
    ...
    try {
        ...
        const result = await Staff.find(filter).populate("terminal_id").populate("office_id").populate("role").populate("bank_id").populate("vehicle_id").populate("asset_assigment_ids").populate("rating_ids").populate("state_id").populate("county_id").skip(skip).limit(limit).sort(sort).select(projection).exec();
        if (!result) {
...
export async function login(req, res) {
    ...
    return staffAuthenticate(req.body).then(({ token, user }) => success(res, 201, { token, user }, "Login was successful!")).catch(err => fail(res, 500, err.message));
}
