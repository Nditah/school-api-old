...
export async function createRecord(req, res) {
    ...
    try {
        ...
        switch (recipient.toUpperCase()) {
            case "STAFF":
                Recipient = Staff;recipientId = staffId;break;
            case "PARTNER":
                Recipient = Partner;recipientId = partnerId;break;
            case "SUPPLIER":
                Recipient = Supplier;recipientId = driverId;break;
            case "CUSTOMER":
                Recipient = Customer;recipientId = customerId;break;
            // case "SUPPLIER": Recipient = Supplier; recipientId = supplierId; break;
            default:
                return fail(res, 422, `Error invalid user type: ${recipient}`);
        }
        ...
        switch (sender.toUpperCase()) {
            case "STAFF":
                Sender = Staff;break;
            case "PARTNER":
                Sender = Partner;break;
            case "SUPPLIER":
                Sender = Supplier;break;
            case "CUSTOMER":
                Sender = Customer;break;
            // case "SUPPLIER": Recipient = Supplier; recipientId = supplierId; break;
            default:
                return fail(res, 422, `Error user type: ${sender}`);
        }
