...
export async function setCollection(req, res) {
    ...
    try {
        ...
        switch (collection) {
            case "account_class":
                Model = AccountClass;break;
            case "account_heading":
                Model = AccountHeading;break;
            case "bank":
                Model = Bank;break;
            case "bank_account":
                Model = BankAccount;break;
            case "budget":
                Model = Budget;break;
            case "category":
            ...
            case "report":
                Model = Report;break;
            case "document_type":
                Model = DocumentType;break;
            case "offence_type":
                Model = OffenceType;break;
            case "material":
                Model = Material;break;
            case "stage":
                Model = Stage;break;
            default:
                return fail(res, 401, `Error invalid collection: ${collection}`);
        }
...
export async function uploadCsv(req, res) {
    ...
    try {
        ...
        const records = [];
        return csv.fromString(csvString, { headers: true, ignoreEmpty: true }).on("data", data => {
            data._id = new mongoose.Types.ObjectId();
            records.push(data);
        }).on("end", () => Model.create(records).then(newRecord => success(res, 201, records, `${newRecord.length} ${collection} record(s) created successfully!`)).catch(err => fail(res, 422, `${err.message}`)));
    } catch (error) {
        logger.error(error);
