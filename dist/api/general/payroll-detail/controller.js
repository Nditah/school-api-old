            ...
            while (1) {
                switch (_context.prev = _context.next) {
                    case 0:
                        ...
                        _context.prev = 2;
                        _context.next = 5;
                        return _model2.default.find(filter).populate("staff_id", "-password, -otp").populate("payroll_id").populate({ path: "staff_id", populate: { path: "office_id" } }).populate("created_by", "id surname other_name email phone").populate("updated_by", "id surname other_name email phone").skip(skip).limit(limit).sort(sort).select(projection).exec();
