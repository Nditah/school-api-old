            ...
            while (1) {
                switch (_context.prev = _context.next) {
                    case 0:
                        ...
                        _context.prev = 2;
                        _context.next = 5;
                        return timetable.find(filter).populate("teacher_id").populate("duration").populate({ path: "teacher_id", select: "-password" }).populate({ path: "manager", select: "-password -otp" }).skip(skip).limit(limit).sort(sort).select(projection).exec();
...
// Logging
var logger = _log4js2.default.getLogger("[timetable]");
