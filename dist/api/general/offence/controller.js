...
import Offence, { schemaCreate, schemaUpdate } from "./model";
...
export async function fetchRecord(req, res) {
    ...
    try {
        const result = await Offence.find(filter).populate("staff_id", "id surname other_name email phone").populate("partner_id", "id surname other_name email phone").populate("verdict_by", "id surname other_name email phone").skip(skip).limit(limit).sort(sort).select(projection).exec();
        if (!result) {
...
export async function createRecord(req, res) {
    ...
    const newRecord = new Offence(data);
    try {
