            ...
            while (1) {
                switch (_context.prev = _context.next) {
                    case 0:
                        ...
                        _context.prev = 2;
                        _context.next = 5;
                        return _model2.default.find(filter).populate("name").populate("email").populate("phone").populate("description").populate({ path: "created_by", select: "surname given_name email phone" }).populate({ path: "updated_by", select: "surname given_name email phone" }).skip(skip).limit(limit).sort(sort).select(projection).exec();
...
// Logging
var logger = _log4js2.default.getLogger("[donation]");
_log4js2.default.configure({
    appenders: { file: { type: "file", filename: "logs/donation.log" } },
