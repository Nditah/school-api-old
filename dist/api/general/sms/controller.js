...
export async function fetchRecord(req, res) {
    ...
    try {
        const result = await Sms.find(filter).populate("created_by", "id phone email type surname other_name").skip(skip).limit(limit).sort(sort).select(projection).exec();
        if (!result) {
    ...
    try {
        ...
        switch (userType) {
            ...
            case "STUDENT":
                Model = Student;break;
            default:
                return fail(res, 422, `Invalid user_type ${userType}`);
        }
...
export async function createRecord(req, res) {
    ...
    try {
        ...
        const result2 = await Staff.findOneAndUpdate({ _id: data.created_by }, data2, { new: true });
