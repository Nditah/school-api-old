...
export async function fetchRecord(req, res) {
    ...
    try {
        const result = await Rating.find(filter).populate("staff_id", "id, surname, other_name").populate("partner_id", "id, surname, other_name, phone, email").populate("vehicle_id", "id, name").populate("terminal_id").skip(skip).limit(limit).sort(sort).select(projection).exec();
        if (!result) {
...
export async function createRecord(req, res) {
    ...
    try {
        ...
        switch (result.subject) {
            case "STAFF":
                Model = Staff;subjectId = "staff_id";
                break;
            case "VEHICLE":
                Model = Vehicle;subjectId = "vehicle_id";
                break;
            default:
                return fail(res, 422, `Unknown subject. ${result.subject}`);
        }
        const result2 = await Model.update({ _id: result[subjectId] }, { $push: { rating_ids: result._id } }).exec();
...
export async function deleteRecord(req, res) {
    ...
    try {
        ...
        const result2 = await Model.update({ _id: result[subjectId] }, { $pull: { rating_ids: result._id } }).exec();
