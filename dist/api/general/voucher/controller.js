...
export async function fetchRecord(req, res) {
    ...
    try {
        const result = await Voucher.find(filter).populate("stage_id").populate("account_heading_id").populate({ path: "staff_id", select: "-password -otp" }).populate({ path: "partner_id", select: "-password -otp" }).populate({ path: "created_by", select: "surname other_name email phone" }).populate({ path: "updated_by", select: "surname other_name email phone" }).skip(skip).limit(limit).sort(sort).select(projection).exec();
        if (!result) {
