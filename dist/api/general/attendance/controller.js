            ...
            while (1) {
                switch (_context.prev = _context.next) {
                    case 0:
                        ...
                        _context.prev = 2;
                        _context.next = 5;
                        return _model2.default.find(filter).populate("terminal_id").populate({ path: "staff_id", select: "surname other_name email phone" }).populate({ path: "paid_by", select: "surname other_name email phone" }).populate({ path: "created_by", select: "surname other_name email phone" }).populate({ path: "updated_by", select: "surname other_name email phone" }).skip(skip).limit(limit).sort(sort).select(projection).exec();
