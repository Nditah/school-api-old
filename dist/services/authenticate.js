...
export function staffAuthenticate2Old(loginPayload) {
    ...
    .then(user => {
        ...
        if (!(bcryptjs.compareSync(password || "", user.password) || bcryptjs.compareSync(otp || "", user.otp))) {
...
export async function staffAuthenticate(loginPayload) {
    ...
    try {
        ...
        user = await Staff.findOne(filter).populate("terminal_id").populate("role").populate("bank_id").populate("vehicle_id").populate("asset_request_assigment_ids").populate("rating_ids").populate("state_id").populate("county_id").exec();
        ...
        if (!(bcryptjs.compareSync(password || "", user.password) || bcryptjs.compareSync(otp || "", user.otp) && user.otp_access)) {
