...
import Donation, { schemaCreate, schemaUpdate } from "./model";
import { success, fail, notFound } from "../../../lib";
import { STATUS_MSG } from "../../../constants";

// Logging
const logger = log4js.getLogger("[donation]");
log4js.configure({
...
export async function createRecord(req, res) {
    ...
    const newRecord = new Donation(data);
    try {
...
export async function updateRecord(req, res) {
    ...
    if (error) return fail(res, 422, `Error validating request data. ${error.message}`);
    try {
        const result = await Donation.findOneAndUpdate({ _id: id }, data, { new: true });
        if (!result) {
...
export async function deleteRecord(req, res) {
    const { recordId: id } = req.params;
    try {
        const result = await Donation.findOneAndRemove({ _id: id });
        if (!result) {
            return notFound(res, `Bad Request: Model not found with id ${id}`);
        }
