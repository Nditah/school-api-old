...
import Attendance, { schemaCreate, schemaUpdate } from "./model";
import { success, fail, notFound, isObjecId } from "../../../lib";
import { STATUS_MSG } from "../../../constants";

// Logging
const logger = log4js.getLogger("[attendance]");
log4js.configure({
...
export async function fetchRecord(req, res) {
    ...
    try {
        const result = await Attendance.find(filter)
            ...
            .populate({ path: "paid_by", select: "surname other_name email phone" })
...
export async function updateRecord(req, res) {
    ...
    if (error) return fail(res, 422, `Error validating request data. ${error.message}`);
    try {
        const result = await Attendance.findOneAndUpdate({ _id: id }, data, { new: true });
        if (!result) {
