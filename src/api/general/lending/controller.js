...
import Lending, { schemaCreate, schemaUpdate } from "./model";
import { success, fail, notFound, isObjecId } from "../../../lib";
import { STATUS_MSG } from "../../../constants";

// Logging
const logger = log4js.getLogger("[lending]");
log4js.configure({
    appenders: { file: { type: "file", filename: "logs/lending.log" } },
...
export async function fetchRecord(req, res) {
    ...
    try {
        const result = await Lending.find(filter)
            .populate("stage_id")
...
export async function createRecord(req, res) {
    ...
    const newRecord = new Lending(data);
    try {
...
export async function deleteRecord(req, res) {
    const { recordId: id } = req.params;
    try {
        const result = await Lending.findOneAndRemove({ _id: id });
        if (!result) {
            return notFound(res, `Bad Request: Model not found with id ${id}`);
        }
